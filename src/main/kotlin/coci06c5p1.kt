...
// 1. var temp ="" 를 상위 스코프에서 변수로 저장할 필요가 없다
// 2. 반복문으로 input을 돌기전에, input의 유효성검사를 "먼저" 수행 => 더욱 안전한 거 같다.
// 3. when(n) 형식으로 무조건 파라미터를 넣어야 하는 줄 알고, 어쩔 수 없이 if문을 사용했다.. when{...}도 가능하구나
// 4. 나는 왜 각 if문 안에서 output.write()을 호출했을까..?